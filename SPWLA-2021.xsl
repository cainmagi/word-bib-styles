...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
  ...
  <xsl:template name="templ_str_EditorsUnCap" >
		<xsl:param name="LCID" />
		<xsl:variable name="_LCID">
			<xsl:call-template name="localLCID">
				<xsl:with-param name="LCID" select="$LCID"/>
		...
		<xsl:value-of select="translate(/*/b:Locals/b:Local[@LCID=$_LCID]/b:Strings/b:EditorsCap, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')"/>
	</xsl:template>
	...
	<xsl:template name ="DisplayPageOrPages">
		...
		<xsl:choose>
			<xsl:when test="contains($pages, '-')">
				<xsl:value-of select="translate($pages,'-','–')"/>
			</xsl:when>
			<xsl:when test="contains($pages, ',')">
		...
		<xsl:if test ="$cAuthor>0 or $cCorpAuthor>0">
			<xsl:choose>
				<xsl:when test="$cEditor = 1">
					...
					<xsl:call-template name ="templ_str_EditorsUnCap"/>
          <xsl:call-template name ="templ_prop_ListSeparator"/>
					<xsl:call-template name ="templ_prop_Space"/>
				</xsl:when>
	...
	<xsl:template name ="BibDisplayVolumeIssue">
		...
		<xsl:variable name="str_VolumeShortCap">
			...
			<xsl:when test ="$cVolume!=0">
				<xsl:choose>
					<xsl:when test ="$cIssue=0">
						<xsl:call-template name ="FindReplaceString">
							<xsl:with-param name="originalString" select="$str_VolumeShortCap"/>
							<xsl:with-param name="stringToBeReplaced" select="'%1'"/>
	...
	<xsl:template name ="BibDisplayJournalName">
      ...
      <xsl:choose>
				<xsl:when test="$cMonth!=0 or $cPages!=0 or $cVolume!=0 or $cIssue!=0 or $cDOI!=0">
					<xsl:call-template name ="templ_prop_ListSeparator"/>
					<xsl:call-template name ="templ_prop_Space"/>
				</xsl:when>
				<xsl:otherwise>
  ...
  <xsl:template name="BibDisplayCityCustomized">
		<xsl:variable name="cCity">
			<xsl:value-of select="count(b:City)"/>
		</xsl:variable>
    ...
    <xsl:variable name="cStandardNumber">
			<xsl:value-of select="count(b:StandardNumber)"/>
		</xsl:variable>
    <xsl:if test ="$cCity!=0 or $cState!=0">
      <xsl:if test ="$cPublisher!=0">
        <xsl:call-template name ="templ_prop_ListSeparator"/>
        <xsl:call-template name ="templ_prop_Space"/>
      </xsl:if>
    </xsl:if>
    ...
    <xsl:if test ="$cStandardNumber!=0">
      <xsl:if test ="$cCity!=0 or $cState!=0 or $cPublisher!=0">
        <xsl:call-template name ="templ_prop_Dot"/>
        <xsl:call-template name ="templ_prop_Space"/>
      </xsl:if>
		</xsl:if>
  ...
  <xsl:template name ="BibDisplayPagesBC">
		<xsl:variable name="cPages">
			<xsl:value-of select="count(b:Pages)"/>
		</xsl:variable>
    <xsl:variable name="cCity">
    ...
    <xsl:variable name="cPublisher">
      ...
      <xsl:choose>
        <xsl:when test ="$cCity!=0 or $cStateProvince!=0 or $cStandardNumber!=0 or $cEdition!=0 or $cPublisher!=0">
            <xsl:call-template name ="templ_prop_ListSeparator"/>
            <xsl:call-template name ="templ_prop_Space"/>
        </xsl:when>
        <xsl:otherwise>
    ...
    <xsl:template name="BibDisplayConfPublication">
		<xsl:variable name="cPublication">
		...
		<xsl:choose>
			<xsl:when test ="$cPublication!=0">
        <xsl:value-of select="b:ConferenceName"/>
        <xsl:choose>
					<xsl:when test ="$cPages!=0 or $cDOI!=0">
						<xsl:call-template name ="templ_prop_ListSeparator"/>
						<xsl:call-template name ="templ_prop_Space"/>
					</xsl:when>
					<xsl:otherwise>
  ...
  <xsl:template name="BibDisplayDayAccessed">
  ...
  <xsl:template name="BibDisplayPagesAndDayMonthAccessed">
    <xsl:variable name="cPages">
			<xsl:value-of select="count(b:Pages)"/>
		</xsl:variable>
    <xsl:variable name="cDOI">
    ...
    <xsl:if test ="$cPages!=0">
      ...
      <xsl:if test ="$cMonthAccessed!=0">
        <xsl:call-template name ="templ_prop_Space"/>
      </xsl:if>
		</xsl:if>
    <xsl:if test="$cMonthAccessed!=0">
      <xsl:value-of select="b:MonthAccessed"/>
      <xsl:if test="$cDayAccessed!=0">
        <xsl:call-template name ="templ_prop_Space"/>
        <xsl:value-of select="b:DayAccessed"/>
      </xsl:if>
    </xsl:if>

    <xsl:if test ="$cPages!=0 or $cMonthAccessed!=0">
      <xsl:choose>
        <xsl:when test ="$cDOI!=0">
            <xsl:call-template name ="templ_prop_ListSeparator"/>
            <xsl:call-template name ="templ_prop_Space"/>
  ...
  <xsl:template name="BibDisplayDOI">
    ...
    <xsl:variable name="urlDOI">
        <xsl:choose>
            <xsl:when test="starts-with(b:DOI,'http')">
                <xsl:value-of select="b:DOI"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="concat('http://dx.doi.org/', b:DOI)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
		<xsl:choose>
  ...
  <xsl:template name="BibDisplayStandardNumber">
    <xsl:variable name="cStandardNumber">
			<xsl:value-of select="count(b:StandardNumber)"/>
		</xsl:variable>
		<xsl:choose>
			<xsl:when test ="$cStandardNumber!=0">
    ...
    <xsl:variable name="vURL">
		...
		<xsl:if test ="$cURL!=0">
			<br>
				<xsl:call-template name ="templ_prop_Space"/>
        <xsl:choose>
          <xsl:when test="starts-with(b:URL,'http')">
            <a href="{$vURL}">
              <xsl:value-of select="b:URL"/>
            </a>
          </xsl:when>
	...
	<xsl:template name ="ProceedBibliography">
      ...
      <xsl:variable name="cDay">
            ...
            <xsl:call-template name = "BibDisplayCityCustomized"/>
            <xsl:call-template name = "BibDisplayStandardNumber"/>
            <xsl:call-template name = "templ_prop_Dot"/>
					</xsl:element>
				</xsl:when>
            ...
            <xsl:call-template name = "BibDisplayPagesBC"/>
						<xsl:call-template name = "BibDisplayEdition"/>
						<xsl:call-template name = "BibDisplayPublisher"/>
            <xsl:call-template name = "BibDisplayCityCustomized"/>
            <xsl:call-template name ="templ_prop_Dot"/>
            ...
            <xsl:call-template name ="BibDisplayDOI"/>
				...
				<xsl:when test="$SourceType = 'ConferenceProceedings'">
					<xsl:element name="p">
						...
						<xsl:call-template name = "BibDisplayTitle"/>
            <i>
              <xsl:call-template name = "BibDisplayConfPublication"/>
            </i>
            <xsl:call-template name = "BibDisplayConfCityConfProc"/>
            <xsl:call-template name = "BibDisplayPagesAndDayMonthAccessed"/>
            <xsl:call-template name = "BibDisplayDOI"/>
					</xsl:element>
				</xsl:when>
